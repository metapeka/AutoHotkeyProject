...
class ProcessManager {
    ...
    StartScript(ScriptName, ScriptPath) {
        try {
            ...
            ; В AutoHotkey v2 используем правильное экранирование кавычек
            try {
    ...
    StopScript(ScriptName) {
        try {
            ...
            if (ProcessExist(Info.PID)) {
                ...
                ; Создаем объект команды вручную
                CmdObj := {command: "SHUTDOWN", id: "shutdown_" . A_TickCount}
                IPCProtocol.SendCommand(Info.PID, CmdObj)
